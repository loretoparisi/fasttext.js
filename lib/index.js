            ...
            this.learn = function (args) {
                ...
                return new Promise(function (resolve, reject) {
                    ...
                    var onErrorDataAppendCallback = function (data) {
                        if (data.indexOf('Number of') > -1) {
                        ...
                        else if (data.indexOf('rogress') > -1) {
                            ...
                            if (evaldata && evaldata.length > 1) {
                                ...
                                for(var p=0; p<evaldata.length; p++) {
                                    if( evaldata[p].indexOf("loss")>-1 && evaldata[p+1]) res['loss']=parseFloat(evaldata[p+1]);
                                    if( evaldata[p].indexOf("lr")>-1 && evaldata[p+1]) res['lr']=parseFloat(evaldata[p+1]);
                                    if( evaldata[p].indexOf("rogress")>-1 && evaldata[p+1]) res['progress']=parseFloat(evaldata[p+1]);
                                    if( evaldata[p].indexOf("words")>-1 && evaldata[p+1]) res['words']=parseFloat(evaldata[p+1]);
                                    if( evaldata[p].indexOf("ETA")>-1 && evaldata[p+1] && evaldata[p+2]) {
                                        res['eta']= evaldata[p+1]+ evaldata[p+2];
                                        res['eta_msec']= Util.ETAtoSeconds( evaldata[p+1]+ evaldata[p+2] )
                                    }
                                }
                                if (self._options.trainCallback) {
                                    if(!Util.empty(res)) self._options.trainCallback.apply(self, [res]);
                                }
                            }
                        }
                    }
